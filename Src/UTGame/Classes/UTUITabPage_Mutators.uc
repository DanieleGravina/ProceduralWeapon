...
function AddMutatorAndFilterList(int NewMutator)
{
	...
	{
		LogInternal("Filtering group: '" $ StringValue $ "'");
...
{
	LogInternal("UTUITabPage_Mutators::SetMutatorEnabled() - MutatorId: "$MutatorId$", bEnabled: "$bEnabled);
...
function OnConfigureMutator()
{
	...
	if(class'UTUIMenuList'.static.GetCellFieldString(EnabledList, 'UIConfigScene', EnabledList.GetCurrentItem(), ConfigureSceneName))
	{
		...
		{
			LogInternal("UTUITabPage_Mutators::OnConfigureMutator() - Unable to find scene '"$ConfigureSceneName$"'");
		}
	}
}
