...
function ApplyLinkSetup(name SetupName)
{
	...
	if (i != INDEX_NONE)
	{
		LogInternal("Activating link setup" @ SetupName);
		...
		if (WI.NetMode != NM_Client)
		{
			...
			for (j = 0; j < LinkSetups[i].NodeLinks.length; j++)
			{
				if (LinkSetups[i].NodeLinks[j].FromNode != None && LinkSetups[i].NodeLinks[j].ToNode != None)
				{
					...
					if (!bFoundEmptySpace)
					{
						WarnInternal("Link setup" @ SetupName $ ": Too many links specified for" @ LinkSetups[i].NodeLinks[j].FromNode);
					}
				}
			}
		}
	...
	{
		WarnInternal("Failed to find link setup" @ SetupName);
...
function RemoveLinkSetup()
{
	...
	if (ActiveSetupName != 'None')
	{
		...
		if (!bFound)
		{
			WarnInternal("Failed to remove link setup" @ ActiveSetupName @ "- could not be found in LinkSetups array");
		}
