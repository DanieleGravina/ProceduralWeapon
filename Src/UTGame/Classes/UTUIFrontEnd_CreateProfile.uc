	...
	if(ValidateUserName(UserName))
	{
		if(Len(Password) > 0)
		{
			if(Password==PasswordConfirm)
			{
				...
				{
					LogInternal("UTUIFrontEnd_CreateProfile::OnCreateProfile() - Creating profile for player with name '"$UserName$"'");
					...
					{
						LogInternal("UTUIFrontEnd_CreateProfile::OnCreateProfile() - Unable to find account interface!");
					}
				}
				else
				{
