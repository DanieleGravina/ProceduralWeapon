...
function int FindBestServerIndexByRanking(int PlayerRanking)
{
	...
	LogInternal(Name$"::"$GetFuncName()@"Rating:"@PlayerRanking@ServerList.Items.length@"Servers available",'DevOnline');
	...
	for(SearchIdx=0; SearchIdx<ServerList.Items.length; SearchIdx++)
	{
		...
		{
			LogInternal(GameToJoin.GameSettings.OwningPlayerName@"Ping ("$"LI:"$SearchIdx@"DSI:"$ServerList.Items[SearchIdx]$"): "$GameToJoin.GameSettings.PingInMs@"Skill"@GameToJoin.GameSettings.AverageSkillRating,'DevOnline');
		}
	}
	...
	if (BestDeltaIndex >= 0)
	{
		LogInternal("Found server"@BestDeltaIndex@"with best score"@BestScore,'DevOnline');
		return BestDeltaIndex;
	}
	else if (BestPingIndex >=0)
	{
		LogInternal("Found server"@BestPingIndex@"with best ping"@BestPing,'DevOnline');
		return BestPingIndex;
	}

	//Fail safe just pick first one
	LogInternal("Failed to find an appropriate server, returning 0",'DevOnline');
	return 0;
}
...
defaultproperties
{
   ...
   Name="Default__UTUITabPage_FindQuickMatch"
