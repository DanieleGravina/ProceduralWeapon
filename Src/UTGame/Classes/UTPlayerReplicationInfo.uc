...
function StopDrivingStat(name NewStatName)
{
	...
	WarnInternal("Stopped driving stat "$NewStatName$" without starting it");
	return;
}
...
function StopPowerupTimeStat(name NewStatName)
{
	...
	// didn't find it - just fail
	WarnInternal("Stopped powerup time stat "$NewStatName$" without starting it");
	return;
}
...
simulated function RenderMapIcon(UTMapInfo MP, Canvas Canvas, UTPlayerController HUDPlayerOwner, LinearColor FinalColor)
{
	...
	else
	{
		...
		{
			LogInternal(PlayerName$" PRI has bad pawn class "$HUDPawnClass);
			return;
		}
	}
...
defaultproperties
{
   ...
   WeaponAwardIndex=-1
   StringUnknown="Centro del campo"
