...
static function bool CheckLikeTheBackOfMyHandAchievement(UTPlayerController PC, INT index)
{
	...
	if (index < 0)
	{
		...
		LogInternal("No pickups for map "$PC.WorldInfo.GetMapName()$" - Marking it completed for Like the Back of my Hand Achievement");
	...
	if (UTPlayerReplicationInfo(PC.PlayerReplicationInfo) != None)
	{
		...
		if ( UTPRI.PickupFlags.Length == Offset+1)
		{
			...
			if (bAllPickups==true)
			{
				LogInternal("You collected all the pickups for map "$PC.WorldInfo.GetMapName()$" - Marking it completed for Like the Back of my Hand Achievement");
				PC.ClientIncrementLikeTheBackOfMyHand(MapIndex);
			}
		}
	}
...
function CheckMissionAchievements()
{
	...
	if ( MissionResult )
	{
		...
		if (UnlockChapter != -1)
		{
			LogInternal("Unlock Chapter "$UnlockChapter);
	...
	if (OnlineSub != None && OnlineSub.StatsInterface != None)
	{
		...
		foreach WorldInfo.AllControllers(class'UTPlayerController',PC)
		{
			...
			if (PRI != None && !PRI.bOnlySpectator && (PRI.UniqueId != ZeroUniqueId))
			{
				...
				{
					LogInternal("Writing out stats for player"@PRI.PlayerName);
			...
			{
				LogInternal("Player"@PRI.PlayerName@"did not have a valid UniqueID, stats not written");
			}
		}
		...
		for (i=0; i<NumInactives; i++)
		{
			...
			{
				LogInternal("Writing out stats for inactive player"@PRI.PlayerName);
				//Write out all relevant stats
				Stats = UTLeaderboardWriteDM(new OnlineStatsWriteClass);
			...
			{
				LogInternal("Inactive player"@PRI.PlayerName@"did not have a valid UniqueID, stats not written");
			}
		}
	}
}
...
defaultproperties
{
   ...
   Acronym="DM"
   Description="Tutti-contro-tutti uccidi o vieni ucciso.  Il giocatore che esegue più frag vince."
   DefaultEnemyRosterClass="UTGame.UTDMRoster"
   MapPrefixes(0)="DM"
   EndOfMatchRulesTemplateStr_Scoring="Chi fa `g frag vince"
   EndOfMatchRulesTemplateStr_ScoringSingle="Il primo che frag vince"
   EndOfMatchRulesTemplateStr_Time="più frag in `t min. vince"
   GameName="Deathmatch"
