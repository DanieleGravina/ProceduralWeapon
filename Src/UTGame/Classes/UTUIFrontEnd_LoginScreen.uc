...
function OnLogin(optional bool bLocalLogin=false)
{
	...
	*/
	...
	if(Len(UserName) > 0 || bLocalLogin)
	{
		...
		{
			// Try logging in
			LogInternal("UTUIFrontEnd_LoginScreen::OnLogin() - Attempting to login with name '"$UserName$"' bLocalOnly:"@bLocalLogin);
			...
			if(PlayerInt!=none)
			{
				...
				{
					LogInternal("UTUIFrontEnd_LoginScreen::OnLogin() - Login call failed.");
					//OnLoginCancelled();
				}
			}
			else
			{
				LogInternal("UTUIFrontEnd_LoginScreen::OnLogin() - Failed to find player interface.");
			}
		}
		else
		{
