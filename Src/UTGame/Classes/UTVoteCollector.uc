...
function Initialize(array<string> MapList)
{
	...
	if (bAllowGameVoting)
	{
		...
		if (k > 1)
		{
			...
			{
				LogInternal("Game voting disabled, as there are not enough valid game profiles",'UTVoting');
				bAllowGameVoting = False;
			}
		}
		else
		{
			LogInternal("Game voting disabled, as the map list manager does not have enough GameProfiles set",'UTVoting');
			bAllowGameVoting = False;
		}
	}
	...
	if (bAllowMutatorVoting)
	{
		...
		for (i=0; i<VotableMutators.Length; ++i)
		{
			...
			{
				LogInternal("Invalid MutClass value for 'VotableMutators' index"@i$", must be in the format: 'Package.MutatorClass'",'UTVoting');
				continue;
			}
			...
			{
				LogInternal("The mutator '"$VotableMutators[i].MutClass$"' is single player only, and can't be added for mutator voting",'UTVoting');
				continue;
			}
			...
			if (VotableMutators[i].MutName == "")
			{
				...
				if (VotableMutators[i].MutName == "")
				{
					LogInternal("Could not load a default name for 'VotableMutators.MutName' at index"@i$", falling back to class name",'UTVoting');
		...
		{
			LogInternal("Mutator voting disabled as there are not enough valid 'VotableMutators' entries",'UTVoting');
...
function AttachVoteReplicationInfo(UTPlayerController PC)
{
	...
	{
		LogInternal("Could not spawn a vote replication info for"@PC.PlayerReplicationInfo.PlayerName@"("$PC$")",'UTVoting');
	}
}
...
state MainVoteRound
{
	function BeginState(name PreviousStateName)
	{
		...
		{
			LogInternal("Invalid 'ActiveGameProfileIdx', getting the current index",'UTVoting');
			ActiveGameProfileIdx = MapListManager.GetCurrentGameProfileIndex();
		}
			...
			if (ActiveGameProfileIdx != INDEX_None)
			{
				...
				if (MLObj != none && MLObj.Maps.Length != 0 && (bAllowGameVoting || MLObj.Maps.Length > 1))
				{
					...
					if (EnabledCount == 0)
					{
						...
						{
							LogInternal("No enabled maps in the current map list, forcibly enabling '"$
								MapVotes[ForceEnableMapIdx].Map$"'",'UTVoting');
						}
						else
						{
							LogInternal("Map voting disabled due to lack of enabled maps",'UTVoting');
				...
				{
					LogInternal("Map voting disabled due to lack of maps",'UTVoting');
					bDisableMapVoting = True;
				}
				else
				{
					LogInternal("Map voting disabled due to an invalid map list name",'UTVoting');
					bDisableMapVoting = True;
				}
			}
			else
			{
				LogInternal("Map voting disabled due to invalid game profile index",'UTVoting');
				bDisableMapVoting = True;
			}
		}
...
function KickVotePassed(int WinIdx)
{
	...
	if (C != none && !C.PlayerReplicationInfo.bAdmin)
	{
		if (PlayerController(C) != none)
		{
			...
			LogInternal("Kick vote passed for player '"$C.PlayerReplicationInfo.GetPlayerAlias()$"'",'UTVoting');
