...
function OnMainRegion_Show_UIAnimEnd(UIObject AnimTarget, int AnimIndex, UIAnimationSeq AnimSeq)
{
	...
	LogInternal(Name$"::"$GetFuncName()@"AnimTarget:"$(AnimTarget!= None ? string(AnimTarget.Name) : "None")@"AnimIndex:'"$AnimIndex$"'"@"AnimSeq:"$(AnimSeq!= None ? string(AnimSeq.Name) : "None"),'DevUI');
...
event SceneActivated(bool bInitialActivation)
{
	...
	LogInternal(Name$"::"$GetFuncName()@"bInitialActivation:'"$bInitialActivation$"'",'DevUI');
	...
	if (IconImages[0] != none)
	{
		...
		LogInternal("Setting image " $ IconImages[IntValue - 1] $ " to visible");
