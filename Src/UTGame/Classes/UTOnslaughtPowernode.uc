	...
	if (Role == ROLE_Authority && StartingOwnerCore != None)
	{
		...
		if (Game != None && StartingOwnerTeam < ArrayCount(Game.Teams))
		{
			...
			if ( ControllingFlag == None )
			{
				...
				if (ControllingFlag == None)
				{
					WarnInternal("No flag for team" @ StartingOwnerTeam);
...
defaultproperties
{
   ...
   PrimeNodeName="Nodo Primario"
   EnemyPrimeNodeName="Nemico Primario"
   ...
   ObjectiveName="Nodo"
   AttackAnnouncement=(AnnouncementText="Attacca il Nodo!")
   DefendAnnouncement=(AnnouncementText="Difendi il Nodo!")
