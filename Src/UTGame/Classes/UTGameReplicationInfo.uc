...
simulated function TickCharacterMeshCreation()
{
	...
	for(i=CharStatus.Length-1; i>=0; i--)
	{
		if(CharStatus[i].MergeState.bMergeInProgress)
		{
			...
			{
				// Merge is done
				LogInternal("CUSTOMCHAR Complete:"@CharStatus[i].PRI@CharStatus[i].PRI.PlayerName@"  (Tex stream:"@(WorldInfo.RealTimeSeconds - CharStatus[i].StartMergeTime)@"Secs)");

				if (CharStatus[i].PRI.Team != None && CharStatus[i].MergeState.TeamString == "V01")
				{
					LogInternal("CUSTOMCHAR Throwing away:"@CharStatus[i].PRI.PlayerName@"should be on"@CharStatus[i].PRI.GetTeamNum() == 1 ? "BLUE" : "RED");
				...
				if(CharStatus[i].PRI.IsLocalPlayerPRI())
				{
						...
						(CharStatus[i].PRI.BlueHeadMIC == None || CharStatus[i].PRI.RedHeadMIC == None) ) // Are missing a skin
					{
						...
						LogInternal("PRI Other Merge Start:" @ CharStatus[i].PRI @ CharStatus[i].PRI.PlayerName @ "Other TeamSkin:" @ TeamString);
		...
		for(i=0; i<CharStatus.Length && !bMergePending; i++)
		{
			...
			if(CharStatus[i].AssetStore != None)
			{
				...
				if(CharStatus[i].AssetStore.NumPendingPackages == 0)
				{
					...
					LogInternal("CUSTOMCHAR Start:" @ CharStatus[i].PRI @ CharStatus[i].PRI.PlayerName @ "TeamSkin:" @ TeamString);
		...
		if(ActiveAssetStore == None)
		{
			...
			else
			{
				...
				if(NextCharIndex != INDEX_NONE)
				{
					...
					LogInternal(CharStatus[NextCharIndex].PRI.PlayerName@"CustomChar - Load Assets:"@LoadFamily);
				...
				else
				{
					...
					LogInternal(CharStatus[0].PRI.PlayerName@"CustomChar - Load Arms:"@LoadFamily);
...
function AssignSinglePlayerCharacters(UTPlayerReplicationInfo PRI)
{
	...
	if (PC != None && !PRI.bOnlySpectator)
	{
		...
		if ( PRI.SinglePlayerCharacterIndex < 0 )
		{
			LogInternal("Server has found an additional Player.  Setting to Spectator!");
...
//			CurrentMidGameMenu.ActivateTab(TabTag);
	...
	if ( Template != none )
	{
		...
		{
			LogInternal("ERROR - Could not open the mid-game menu:"@Template);
		}
	}
