...
simulated function PostBeginPlay()
{
	...
	if ( MissionInfoClass != none )
	{
		...
		if ( MissionData == none )
		{
			LogInternal("WARNING: Could not create an instance of the Mission Info. A lot will be broken!  Epic please fix");
		}
	}
	else
	{
		LogInternal("WARNING: Could not create the mission info class ("$MissionInfoClassName$").  A lot will be broken!");
	}
		...
		if ( MI != INDEX_NONE )
		{
			if (MissionData.Missions[MI].GlobeBoneName == '')
			{
				...
				if (s != "")
				{
					...
					LogInternal("Fixing up Map Point Bone Name:"@s);
...
//			`log("### Adding a Mission"@ClientMissionTagList.Data[i]);
		...
		{
			LogInternal("The Game is trying to send too many missions!");
		}
