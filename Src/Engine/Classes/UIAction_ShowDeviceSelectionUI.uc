	...
	if (OnlineSub != None)
	{
		...
		if (PlayerIntEx != None)
		{
			...
			if (PlayerIntEx.ShowDeviceSelectionUI(class'UIInteraction'.static.GetPlayerControllerId(PlayerIndex),SizeNeeded,bForceShowUI) == false)
			{
				...
				LogInternal("Error occured while trying to display device selection UI");
		...
		else
		{
			// Don't wait if there was an error
			bIsDone = true;
			LogInternal("No OnlinePlayerInterfaceEx present to display the device selection UI");
		}
	}
...
function OnDeviceSelectionComplete(bool bWasSuccessful)
{
	...
	LogInternal("Device Result: " $ bWasSuccessful);
	bIsDone = true;
