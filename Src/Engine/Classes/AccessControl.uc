...
function SessionBanPlayer(PlayerController C)
{
	...
	if (C != none && !IsAdmin(C) && NetConnection(C.Player) != none)
	{
		...
		LogInternal("Session banning '"$C.PlayerReplicationInfo.GetPlayerAlias()$"', IP:"@SB.BanIP$", Hash:"@SB.BanHash$", ID:"@
...
function bool CheckIPPolicy(string Address)
{
	...
	{
		LogInternal("Denied connection for "$Address$" due to session ban");
...
defaultproperties
{
   ...
   BannedCDHashKeyString="Codice CD bandito."
   TimedOutCDHashKeyString="Controllo del codice con il server fallito. Aggiornare con l'ultima patch."
