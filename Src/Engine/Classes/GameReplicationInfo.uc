...
{
	LogInternal(Name$"::"$GetFuncName(),'DataStore');
	if ( CurrentGameData != None )
	{
...
simulated function OnlineGameInterface GetOnlineGameInterface()
{
	...
	{
		LogInternal("Could not get Online SubSystem");
	}
	return none;
}
	...
	if ( bForced || WorldInfo.NetMode != NM_StandAlone )
	{
		...
		if (bNeedsOnlineCleanup)
		{
			...
			if (GI != None && GI.GetOnlineGameState() != OGS_NoSession)
			{
				...
				LogInternal("GameReplicationInfo::OnlineSession_EndSession() - Destroying Online Game");
...
function OnDestroyOnlineGameComplete(bool bWasSuccessful)
{
	...
	LogInternal("GameReplicationInfo::OnDestroyOnlineGameComplete() bWasSuccesful:"@bWasSuccessful);
	GI = GetOnlineGameInterface();
	if (GI != None)
	{
