...
function LogRenderBounds( int Indent )
{
	...
	LogInternal(IndentString $ "'" $ WidgetTag $ "': (" $ RenderBounds[0] $ "," $ RenderBounds[1] $ "," $ RenderBounds[2] $ "," $ RenderBounds[3] $ ") Pos:(" $ Position.Value[0] $ "," $ Position.Value[1] $ "," $ Position.Value[2] $ "," $ Position.Value[3] $ ")");
						...
						optional float PlaybackRate=1.0,optional bool bLoop, optional float InitialPosition=0.0)
{
	...
	if ( AnimName != '' )
	{
		LogInternal("Playing animation '" $ AnimName $ "' on " $ WidgetTag $ ":" @ PlaybackRate $ "," @ bLoop $ "," @ InitialPosition,'DevUIAnimation');
...
*/
				LogInternal("Animation found at" @ Idx,'DevUIAnimation');
...
*/
	...
	{
		LogInternal("UIAnimation - Attempted to play unknown sequence '"$AnimName$"' on "@WidgetTag);
	}
}
...
event StopUIAnimation(name AnimName, optional UIAnimationSeq AnimSeq, optional bool bFinalize)
{
	...
	for ( Idx=0; Idx < AnimStack.Length; Idx++)
	{
		...
		{
			LogInternal("Stopping animation" @ Idx @ "for" @ PathName(Self) $ ":" @ "AnimName:'"$AnimName$"'" @ "AnimSeq:"$(AnimSeq!= None ? string(AnimSeq.Name) : "None") @ "bFinalize:'"$bFinalize$"'",'DevUIAnimation');
			if (bFinalize)
			{
...
event ClearUIAnimation(name AnimName, optional UIAnimationSeq AnimSeq)
{
	...
	LogInternal("Clearing animations for" @ PathName(Self) $ ":" @ "AnimName:'"$AnimName$"'" @ "AnimSeq:"$(AnimSeq!= None ? string(AnimSeq.Name) : "None"),'DevUIAnimation');
	for ( Idx=0; Idx < AnimStack.Length; Idx++)
	{
		if ((AnimName != '' && AnimStack[Idx].SeqRef.SeqName == AnimName)
		||	(AnimSeq != none && AnimStack[Idx].SeqRef == AnimSeq) )
...
event UIAnimEnd(int SeqIndex)
{
	...
	if ( SeqIndex >= 0 && SeqIndex < AnimStack.Length )
	{
		...
		{
			LogInternal("Can't unsubscribe" @ PathName(Self) @ "because it still has active animations:" @ AnimStack.Length,'DevUIAnimation');
		}
	}
	else
	{
		WarnInternal("Invalid animation sequence index specified in call to UIAnimEnd for" @ PathName(Self) $ ":" @ SeqIndex);
	}
}
