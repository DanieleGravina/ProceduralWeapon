...
*/
			...
			{
				WarnInternal("OnlineSubsystem does not support the player interface. Can't retrieve recent players list for player ("$
					Player.ControllerId$")");
			}
		}
		else
		{
			WarnInternal("No OnlineSubsystem present. Can't retrieve recent players list for player ("$
				Player.ControllerId$")");
		}
	}
}
