...
final function CreateGameDataProvider( GameReplicationInfo GRI )
{
	if ( GRI != None )
	{
		...
		{
			LogInternal(Name$"::"$GetFuncName()@"Failed to bind GameReplicationInfo to game data store:"@"GRI:"$(GRI!= None ? string(GRI.Name) : "None"),'DevDataStore');
		}
	}
	else
	{
		LogInternal(Name$"::"$GetFuncName()@"NULL GRI specified!"@ "GameData:"$(GameData!= None ? string(GameData.Name) : "None"),'DevDataStore');
	}
}
...
final function AddPlayerDataProvider( PlayerReplicationInfo PRI )
{
	...
	LogInternal(">> CurrentGameDataStore::AddPlayerDataProvider -" @ PRI @ "(" $ PlayerName $ ")",'DevDataStore');
	if ( PRI != None )
	{
		if ( GameData != None )
		{
			...
			if ( ExistingIndex == INDEX_NONE )
			{
				...
				{
					LogInternal("Failed to bind PRI to PlayerDataProvider:"@ DataProvider @ "for player" @ PlayerName,'DevDataStore');
				}
				else
				{
	...
	{
		LogInternal("NULL PRI specified - current number of player data provider:"@ PlayerData.Length,'DevDataStore');
	}
	LogInternal("<< CurrentGameDataStore::AddPlayerDataProvider -" @ PRI @ "(" $ PlayerName $ ")",'DevDataStore');
}
...
final function RemovePlayerDataProvider( PlayerReplicationInfo PRI )
{
	...
	{
		LogInternal(Name$"::"$GetFuncName()@"NULL PRI specified!"@ "PlayerData.Length:'"$PlayerData.Length$"'",'DevDataStore');
	}
}
...
final function AddTeamDataProvider( TeamInfo TI )
{
	...
	LogInternal(">> CurrentGameDataStore::AddTeamDataProvider -" @ TI @ "(" $ TeamName $ ")",'DevDataStore');
	if ( TI != None )
	{
		if ( GameData != None )
		{
			...
			if ( ExistingIndex == INDEX_NONE )
			{
				...
				{
					LogInternal("Failed to bind TeamInfo to TeamDataProvider in 'CurrentGame' data store:" @ DataProvider @ "for team" @ TeamName,'DevDataStore');
				}
				else
				{
	...
	{
		LogInternal("NULL TeamInfo specified - current number of team data providers:"@ TeamData.Length,'DevDataStore');
	}
}
...
final function RemoveTeamDataProvider( TeamInfo TI )
{
	...
	{
		LogInternal(Name$"::"$GetFuncName()@"NULL TeamInfo specified!"@ "TeamCount:'"$TeamData.Length$"'",'DevDataStore');
	}
}
...
final function ClearDataProviders()
{
	...
	LogInternal(">>" @ Name$"::"$GetFuncName(),'DevDataStore');
	if ( GameData != None )
	{
