...
def TuningSingleWeapon(iter = 0):
    ...
    os.chdir("final_simulation_multi_obj")
    os.chdir("error")
    ...
    #plt.plot([i for i in range(100)], [abs(pop[i].fitness.values[0] - pop[i].fitness.values[1]) for i in range(100)])
    #plt.plot([i for i in range(100)], [abs(abs(pop[i].fitness.values[0] - pop[i].fitness.values[1]) - (3 - pop[i].fitness.values[0])) for i in range(100)], "r")
        plt.errorbar([i for i in range(100)], [numpy.mean([dists[i][0], dists[i][1]])  for i in range(p*100, (p+1)*100)], 
           yerr = [numpy.std([dists[i][1], dists[i][0]]) for i in range(p*100, (p+1)*100) ])

    plt.savefig("diff dist.png", bbox_inches='tight', dpi = 400)
    plt.close()
    ...
    #plt.plot([i for i in range(100)], [abs(pop[i].fitness.values[0] - pop[i].fitness.values[1]) for i in range(100)])
    #plt.plot([i for i in range(100)], [abs(abs(pop[i].fitness.values[0] - pop[i].fitness.values[1]) - (3 - pop[i].fitness.values[0])) for i in range(100)], "r")
        plt.errorbar([i for i in range(100)], [numpy.mean([streaks[i][0], streaks[i][1]])  for i in range(p*100, (p+1)*100)], 
           yerr = [numpy.std([streaks[i][1], streaks[i][0]]) for i in range(p*100, (p+1)*100) ])
