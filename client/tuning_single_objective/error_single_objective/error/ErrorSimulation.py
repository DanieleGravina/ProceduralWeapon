...
def getPopulationData(pop):
    ...
    for i in range(10):
        p = toolbox.clone(pop[i*100:(i+1)*100])
...
def TuningSingleWeapon(iter = 0):
    ...
    os.chdir("error_single_objective")
    ...
    #plt.plot([i for i in range(100)], [abs(abs(pop[i].fitness.values[0] - pop[i].fitness.values[1]) - (3 - pop[i].fitness.values[0])) for i in range(100)], "r")
        plt.errorbar([i for i in range(100)], [pop[i].fitness.values[1] for i in range(p*100, (p+1)*100)], 
    	   yerr = [abs(pop[i].fitness.values[1] - pop[i].fitness.values[0]) for i in range(p*100, (p+1)*100) ])

    plt.savefig("err entropy.png", bbox_inches='tight', dpi = 200)
    plt.close()
    ...
    #plt.plot([i for i in range(100)], [abs(pop[i].fitness.values[0] - pop[i].fitness.values[1]) for i in range(100)])
    #plt.plot([i for i in range(100)], [abs(abs(pop[i].fitness.values[0] - pop[i].fitness.values[1]) - (3 - pop[i].fitness.values[0])) for i in range(100)], "r")
        plt.plot([i for i in range(100)], [pop[i].fitness.values[0] for i in range(p*100, (p+1)*100)])
        plt.plot([i for i in range(100)], [pop[i].fitness.values[1] for i in range(p*100, (p+1)*100)], "r")
