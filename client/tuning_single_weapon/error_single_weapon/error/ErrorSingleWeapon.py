...
def TuningSingleWeapon(iter = 0):
    ...
    for p in range(10) :
        ...
        plt.plot([i for i in range(50)], [pop[i].fitness.values[0] for i in range(p*50, (p+1)*50)])
        plt.plot([i for i in range(50)], [pop[i].fitness.values[1] for i in range(p*50, (p+1)*50)], "r")
    ...
    plt.savefig("diff entropy 2.png", bbox_inches='tight', dpi = 200)
    plt.close()
    ##########################################################################
    ...
    plt.plot([i for i in range(NUM_POP - 49)], moving_average([pop_sorted[i].fitness.values[1] for i in range(NUM_POP)]), "r")
